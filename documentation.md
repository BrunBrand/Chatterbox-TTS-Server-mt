...
|                       | `repetition_penalty`          | float         | Multilingual only. Penalizes repeated tokens/syllables (0.0-5.0).                                             | `2.0`                    |
|                       | `top_p`                       | float         | Multilingual only. Nucleus sampling probability mass (0.0-1.0).                                               | `1.0`                    |
|                       | `min_p`                       | float         | Multilingual only. Minimum probability threshold for tokens (0.0-0.5).                                        | `0.05`                   |
...
*   **Request Body (`CustomTTSRequest` from `models.py` [1]):**
    ...
    | `repetition_penalty`        | float \| null                    | No          | Multilingual only. Overrides repetition penalty to curb repeated tokens.                                      | `generation_defaults.repetition_penalty`     |
    | `top_p`                     | float \| null                    | No          | Multilingual only. Overrides top-p (nucleus sampling) probability mass.                                       | `generation_defaults.top_p`                  |
    | `min_p`                     | float \| null                    | No          | Multilingual only. Overrides min-p (minimum token probability) threshold.                                     | `generation_defaults.min_p`                  |
